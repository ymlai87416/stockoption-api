...
public class StockController {
                                            ...
                                            @RequestParam(value="endDate", required=false) String endDate)
    {
        ...
        if(searchResult != null && searchResult.size() > 1 ){
            ...
            if(startEndDate != null && startEndDate.length == 2
                ...
                childSearchResult = dailyPriceRepository.findBySymbolInAndPriceDateBetween(Collections.singletonList(resultSymbol),
                        startEndDate[0], startEndDate[1]);

                long stockId = resultSymbol.getId();
                List<Date> dateList = childSearchResult.stream().map(x -> x.getPriceDate()).collect(Collectors.toList());
                ...
                List<Double> priceList = childSearchResult.stream().map(x -> x.getAdjClosePrice()).collect(Collectors.toList());
                ...
                std.evaluate(priceValues);
                double statStdPrice = std.getResult();

                if(statStartDate.isPresent() && statEndDate.isPresent() && statMaxPrice.isPresent() && statMinPrice.isPresent() && statAvgPrice.isPresent())
    ...
    private List<Stock> convertToStockList(List<Symbol> symbolList, boolean skipChild){
        return symbolList.stream().map(x -> convertToStock(x, skipChild)).collect(Collectors.toList());
    }
